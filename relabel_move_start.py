"""
relabel_move_start.py — New labeling: did a 0.5% move START within 1 hour?

For each window_end_ms, look forward 3600s (1 hour).
- If price moved +0.5% from entry: had_move=1, move_direction=+1
- If price moved -0.5% from entry: had_move=1, move_direction=-1
- If neither: had_move=0, move_direction=0

We are NOT predicting where price ends up. We are predicting whether
the tape triggered a move.
"""
import numpy as np
import pandas as pd
from pathlib import Path

INPUT_FILE = "data/research_dataset_futures.parquet"
OUTPUT_FILE = "data/research_dataset_futures_move_start.parquet"
...
for i in range(n_windows):
    ...
    # Entry price: first trade at or after win_end
    ...
    if hi <= lo:
        continue
    px = trade_px[lo:hi]
    # Max and min price in the lookahead window
    max_px = px.max()
    min_px = px.min()
    max_pct = (max_px - entry) / entry
    min_pct = (min_px - entry) / entry
    # Which threshold hit first?
...
# Set outcome columns for training
# For had_move prediction: outcome_binary = had_move
# For direction prediction: we'll train two models (had_move first, then direction on had_move=1)
