"""
run_research_oos.py — Single-day OOS download + sweep.

Designed to fit in 30GB RAM. Uses small windows/horizons to reduce
sweep memory. Streams book processing to avoid OOM.
"""
import sys
import gc
import time
...
# Smaller window/horizon set to reduce memory
WINDOW_SIZES_SEC = [60]  # only micro
HORIZONS_SEC = [1800, 3600]  # only 30m, 1h
...
def download_day(date_str, day_dir):
    day_dir.mkdir(parents=True, exist_ok=True)
    trades_path = day_dir / "trades.parquet"
    book_path = day_dir / "book.parquet"
    liq_path = day_dir / "liq.parquet"

    if trades_path.exists() and book_path.exists() and liq_path.exists():
        print(f"  all files exist, skip download", flush=True)
        return

    client = chd.CryptoHFTDataClient(api_key=API_KEY, max_workers=1, timeout=120)
    ...
    if not book_path.exists():
        ...
        del df_b
        gc.collect()
        print(f"  book: OK ({n_b:,})", flush=True)
    ...
    if not liq_path.exists():
        ...
        try:
            ...
            if len(df_l) > 0:
                ...
                pd.DataFrame(columns=["timestamp_ms"]).to_parquet(liq_path, index=False)
        except:
            pd.DataFrame(columns=["timestamp_ms"]).to_parquet(liq_path, index=False)
        try:
            del df_l
...
def main(start_str, end_str):
    ...
    while d <= end:
        ...
        if daily_feature_file.exists():
            print(f"  Features cached, loading...", flush=True)
            day_df = pd.read_parquet(daily_feature_file)
            all_results.append(day_df)
        else:
            download_day(date_str, day_dir)
            ...
            print(f"  {len(trades):,} trades, {len(book_df):,} book, {len(liq_df):,} liq", flush=True)
